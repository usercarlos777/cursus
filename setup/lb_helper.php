...
class LicenseBoxAPI{
	...
	public function __construct(){ 
		$this->product_id = '721F39A7';
		$this->api_url = 'https://licencefly.com/';
		$this->api_key = '30B4A3CAA330EC-cursus';
	...
	public function verify_license($time_based_check = false, $license = false, $client = false){
		...
		if($time_based_check && $this->verification_period > 0){
			...
			$today = date('d-m-Y');
			if(empty($_SESSION["e4dfd61344c054e"])){
				$_SESSION["e4dfd61344c054e"] = '00-00-0000';
			}
			if($type == 1){
				$type_text = '1 day';
			}elseif($type == 3){
			...
			if(strtotime($today) >= strtotime($_SESSION["e4dfd61344c054e"])){
				$get_data = $this->call_api(
					'POST',
